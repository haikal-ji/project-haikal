...
export const WarmTooltipGroup = forwardRef<WarmTooltipGroupHandle, WarmTooltipGroupProps>(function WarmTooltipGroup(
  { delay = 300, warmWindow = 300, travel = 280, lean = 0, onWarmChange, children },
  ref
) {
    ...
    <GroupContext.Provider value={group}>
      ...
      {state !== 'closed' && current && canPortal
        ? createPortal(
            ...
            >
              <motion.span
                className="absolute inset-0 rounded-[var(--wt-radius)] [background:var(--wt-surface)] [color:var(--wt-ink)] [box-shadow:0_1px_3px_rgba(0,0,0,0.3),0_8px_24px_-4px_rgba(0,0,0,0.5)] [transform-origin:var(--wt-origin)] border border-white/10"
              ...
              >
                <AnimatePresence initial={false} custom={st.current.swap}>
                  ...
                  >
                    <span
                      ...
                      className="inline-flex items-center gap-[7px] whitespace-nowrap font-medium leading-[1.2] tracking-[-0.01em] [font-size:var(--wt-font)] px-1"
                    >
                      {current.content}
                      {current.shortcut ? (
                        <kbd className="inline-flex h-[1.55em] items-center rounded-[0.4em] px-[0.45em] text-[0.86em] font-mono font-medium tracking-[0.02em] tabular-nums [background:color-mix(in_srgb,var(--wt-ink)_12%,transparent)] [box-shadow:inset_0_-1px_0_color-mix(in_srgb,var(--wt-ink)_15%,transparent)] [color:color-mix(in_srgb,var(--wt-ink)_80%,transparent)]">
                          {current.shortcut}
                        </kbd>
                      ) : null}
                    </span>
                ...
                {current.arrow ? (
                  <motion.span
                    className="absolute -m-1 h-2 w-2 rounded-[2px] [background:var(--wt-surface)] [transform:rotate(45deg)] data-[side=top]:bottom-[1px] data-[side=bottom]:top-[1px] data-[side=left]:right-[1px] data-[side=right]:left-[1px] border-b border-r border-white/10"
                    data-side={side}
                    style={arrowStyle}
                    aria-hidden="true"
                  />
